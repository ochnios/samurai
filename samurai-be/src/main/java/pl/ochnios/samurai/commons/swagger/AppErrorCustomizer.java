...
public class AppErrorCustomizer implements OpenApiCustomizer {
    ...
    private void addAppErrorSchema(OpenAPI openApi) {
        var appErrorSchema = createAppErrorSchema();
    ...
    private Schema<AppError> createAppErrorSchema() {
        var errorIdSchema = new UUIDSchema().description("${docs.dto.error.errorId}");
        var errorsSchema = new ArraySchema()
                .items(new StringSchema())
                .description("${docs.dto.error.errors}")
                .uniqueItems(true);
        var schema = new Schema<AppError>();
    ...
    private void customizeErrorResponse(OpenAPI openApi, String code, String description) {
        var mediaType = new MediaType().schema(new Schema<AppError>().$ref("#/components/schemas/AppError"));
        var content = new Content().addMediaType(APPLICATION_JSON_VALUE, mediaType);
        var response = new ApiResponse().description(description).content(content);
